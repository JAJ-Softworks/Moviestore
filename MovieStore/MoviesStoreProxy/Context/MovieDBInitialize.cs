...
namespace MoviesStoreProxy.Context
{
    public class MovieDBInitialize : DropCreateDatabaseAlways<MovieStoreContext>
    {
        ...
        protected override void Seed(MovieStoreContext context)
        {
            ...
            context.Orders.Add(new Order() { Id = 1, date = DateTime.Now.AddYears(-25), CustomerId = 3, });
            context.Orders.Add(new Order() { Id = 2, date = DateTime.Now.AddYears(-15), CustomerId = 2, });
            context.Orders.Add(new Order() { Id = 3, date = DateTime.Now.AddYears(-10), CustomerId = 1 });
